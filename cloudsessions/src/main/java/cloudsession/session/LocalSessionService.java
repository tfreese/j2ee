...
public class LocalSessionService implements CloudSession
{
    ...
    {
        String key = getPropsKey(sessionID, name);
        String property = getProps().getProperty(key);

        return ObjectSerializer.fromJson(property);
    }
    ...
    public void setSessionValue(final String sessionID, final String name, final Object value)
    {
        ...
        String key = getPropsKey(sessionID, name);
        String json = ObjectSerializer.toJson(value);
